...
public class BearlyDriveServer implements Runnable {
	...
	private String commands = "Drive Forward [distance] ##"
			...
			+ "\tRotates the robot x degrees around its center ##"
			+ "Turn Left ##"
			+ "\tRotates the robot left 90 degrees around its center ##"
			+ "Turn Right ##"
			+ "\tRotates the robot right 90 degrees around its center ##"
			+ "Turn Left [degrees] ##"
			+ "\tRotates the robot left x degrees around its center ##"
			+ "Turn Right [degrees] ##"
			+ "\tRotates the robot right x degrees around its center ##"
			+ "Open/Close/Toggle Claw ##"
			+ "\tOpens, closses, or toggles the state of the elevator's arm ##"
			+ "help ##" + "\tList all the commands and what they do"
			+ "Waypoint [position between 0 - 6] ##"
	...
	public static BearlyDriveServer getInstance() {
		if (instance_ == null)
			try {
				instance_ = new BearlyDriveServer(new ServerSocket(PORT));
			} catch (IOException e) {
	...
	private BearlyDriveServer(ServerSocket socket) {
						...
						if (input.contains("drive forward")) {
							...
							if (input.length() > 10) {
								System.out.println("Turning left "
										+ input.substring(10) + " degress");
								degrees = -Math.abs(Integer.parseInt(input
										.substring(10)));
							} else {
								System.out.println("Turning left");
								degrees = -90;
							...
							if (input.length() > 5) {
								...
								System.out.println("Turning "
										+ Integer.parseInt(input) + " degrees");
								degrees = Integer.parseInt(input);
							}

							// Open claw
						} else if (input.contains("claw")) {
							input = input.trim().substring(0,
									input.length() - 5);
							// open or close
							if (input.contains("open")) {
								...
								System.out
										.println("Toggling the state of the claw");
								claw = !claw;
							} else {
							...
							if (value < 0 || value > 6)
								...
								waypoint = value;
							System.out.println("Moving elevator to waypoint "
									+ value);
